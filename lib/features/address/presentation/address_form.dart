      ...
      items.add(
        DropdownSearch<String>(
          popupProps: PopupProps.menu(
            ...
            itemBuilder:
                (context, value, isSelected) => Padding(
          ...
          items:
              address.availableCountries
                  .build()
                  .map((v) => v.text ?? "")
                  .toList(),
          selectedItem:
              (() {
                final selectedCountry = address.availableCountries
                    .build()
                    .firstWhere(
                      (v) => v.value == address.countryId?.toString(),
                      //orElse: () => null,
                    );
          ...
          dropdownDecoratorProps: DropDownDecoratorProps(
            ...
            dropdownSearchDecoration: InputDecoration(
              hintText: context.locale!.address_form_select_country,
          ...
          onChanged: (String? newText) async {
            final selected = address.availableCountries.build().firstWhere(
              (v) => v.text == newText,
              //orElse: () => null,
            );
            ...
            setState(() {
              address.countryId = int.parse(selected.value!);
            });
      ...
      if (availableStates?.isNotEmpty ?? false) {
        items.add(
          /* DropdownSearch<String>(
            ...
            items: availableStates!.map((v) => v.name ?? "").toList(),
            selectedItem:
                (() {
                  final selectedState = availableStates!.firstWhere(
                    (s) => s.id == address.stateProvinceId,
                  );
            ...
            dropdownDecoratorProps: DropDownDecoratorProps(
              baseStyle: Theme.of(context).textTheme.bodyLarge,
              dropdownSearchDecoration: InputDecoration(
                hintText: context.locale!.address_form_select_state,
                contentPadding: const EdgeInsets.all(15),
                border: const UnderlineInputBorder(
                  borderSide: BorderSide(color: Colors.transparent, width: 1),
                ),
            ...
            onChanged: (String? selectedText) {
              final selectedState = availableStates!.firstWhere(
                (s) => s.name == selectedText,
              );

              _formKey.currentState!.save();
              setState(() {
                address.stateProvinceId = selectedState.id!;
              });
            },
          ) */
          /* DropdownButton(
          ...
          SearchableDropdown(
            ...
            itemLabel: (s) => s.name ?? "",
            selectedValue: address.stateProvinceId,
            isSelected: (s, val) => s.id == val,
            hint: "Search State",
            onItemSelected: (selected) {
              _formKey.currentState!.save();
              setState(() {
                address.stateProvinceId = selected.id!;
              });
            },
          ),
        );
...
class SearchableDropdown<T> extends StatelessWidget {
  ...
  final bool Function(T, dynamic) isSelected;
  ...
  Widget build(BuildContext context) {
    final selectedItem = items.firstWhere(
      (item) => isSelected(item, selectedValue),
      orElse: () => items.first,
    );

    return DropdownSearch<String>(
      items: items.map(itemLabel).toList(),
      selectedItem: selectedItem != null ? itemLabel(selectedItem) : "",
      popupProps: PopupProps.menu(
        ...
        itemBuilder:
            (ctx, label, isSelected) =>
                Padding(padding: const EdgeInsets.all(15), child: Text(label)),
      ),
      dropdownDecoratorProps: DropDownDecoratorProps(
