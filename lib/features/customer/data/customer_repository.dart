...
class CustomerRepository extends BaseRepository {
  Future<CustomerInfoModelDto> getCurrentCustomer() async {
    ...
    try {
      ...
      if ((e as DioException).response?.data.toString() ==
  ...
  Future<CustomerDownloadableProductsModelDto>
      getCurrentCustomerDownloadableProducts() async {
    final api = await WebApiHelper.getApi(
        (frontendApi) => frontendApi.getCustomerApi());
  ...
  Future<CustomerBackInStockSubscriptionsModelDto>
      getCurrentCustomerBackInStock(int? pageNumber) async {
    final api = await WebApiHelper.getApi(
        (frontendApi) => frontendApi.getBackInStockSubscriptionApi());
    final response =
        await api.apiFrontendBackInStockSubscriptionCustomerSubscriptionsGet(
            pageNumber: pageNumber);

    return response.data ?? CustomerBackInStockSubscriptionsModelDto();
  }

  Future<CustomerBackInStockSubscriptionsModelDto> setBackInStockSubscription(
      int productId) async {
    final api = await WebApiHelper.getApi(
        (frontendApi) => frontendApi.getBackInStockSubscriptionApi());
    ...
    final response =
        await api.apiFrontendBackInStockSubscriptionCustomerSubscriptionsPost(
  ...
  Future<CustomerRewardPointsModelDto> getCurrentCustomerRewardPoints(
      int? pageNumber) async {
    final api =
        await WebApiHelper.getApi((frontendApi) => frontendApi.getOrderApi());
    final response = await api.apiFrontendOrderCustomerRewardPointsGet(
        pageNumber: pageNumber);

    return response.data ?? CustomerRewardPointsModelDto();
  }

  Future<CustomerProductReviewsModelDto> getCurrentCustomerProductReviews(
      page) async {
    final api =
        await WebApiHelper.getApi((frontendApi) => frontendApi.getProductApi());
    final response =
        await api.apiFrontendProductCustomerProductReviewsGet(pageNumber: page);

    return response.data ?? CustomerProductReviewsModelDto();
  }

  Future<bool> changeCustomerInfo(
      CustomerInfoModelDtoBuilder customerInfo) async {
    final api = await WebApiHelper.getApi(
        (frontendApi) => frontendApi.getCustomerApi());
    ...
    final response = await api.apiFrontendCustomerInfoPost(
        customerInfoModelDtoBaseModelDtoRequest: builder.build());
  ...
  Future<bool> changePassword(
    ...
    final response = await api.apiFrontendCustomerChangePasswordPost(
        changePasswordModelDto: builder.build());
