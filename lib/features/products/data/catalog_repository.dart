...
class CatalogRepository extends BaseRepository {
  ...
  Future<BuiltList<CategorySimpleModelDto>?> getAllCategories() async {
    final api = await WebApiHelper.getApi(
      (frontendApi) => frontendApi.getCatalogApi(),
    );

    final response = await api.apiFrontendCatalogGetCatalogRootGet(
      loadImage: true,
    );
  ...
  Future<CategoryModelDto?> getCategoryById({
    ...
    CatalogProductsCommandDtoBuilder? pageParams,
  }) async {
    final api = await WebApiHelper.getApi(
      (frontendApi) => frontendApi.getCatalogApi(),
    );
  ...
  ///Get products by Tag
  Future<GetCategoryProductsResponse?> getProductsById({
    required int categoryId,
    CatalogProductsCommandDtoBuilder? pageParams,
  }) async {
    final api = await WebApiHelper.getApi(
  ...
  Future<SearchProductsResponse?> searchProducts({
    required SearchRequestBuilder searchRequest,
  }) async {
    final api = await WebApiHelper.getApi(
      (frontendApi) => frontendApi.getCatalogApi(),
    );
  ...
  Future<BuiltList<SearchTermAutoCompleteResponse>?>
  searchProductsAutocomplete({required String term}) async {
    final api = await WebApiHelper.getApi(
      (frontendApi) => frontendApi.getCatalogApi(),
    );

    final response = await api.apiFrontendCatalogSearchTermAutoCompleteGet(
      term: term,
    );
