...
class _CategoryDetailsScreenState extends ConsumerState<CategoryDetailsScreen> {
  ...
  /* final PagingController<int, ProductOverviewModelDto>
  _pagingController = PagingController<int, ProductOverviewModelDto>(
    getNextPageKey: (state) {
      final lastPage = state.pages!.isNotEmpty ? state.pages!.last : [];
      return lastPage.isEmpty ? null : state.nextIntPageKey;
    },
    fetchPage: (pageKey) async {
      try {
        //final repository = ref.read(getRepositoryProvider(() => ProductRepository()));
        //final result = await repository.getProducts(pageKey);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      //appBar: AppBar(backgroundColor: GlobalVariables.accentColor),
      // #region
      // appBar: AppBar(
      //   title: Text(_title),
      //   leading: IconButton(
            ...
            slivers: [
              SliverAppBar(
                ...
                actions: <Widget>[
                  ...
                  if (hasSortOption(_category?.catalogProductsModel))
                    IconButton(
                      icon: const Icon(Icons.sort, color: Color(0xFFFFFFFF)),
                      onPressed: () {
                        sortOptionsBuilder(
                          context: context,
                          catalogProducts: _category?.catalogProductsModel,
