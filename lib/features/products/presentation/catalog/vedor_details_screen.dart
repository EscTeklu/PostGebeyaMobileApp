...
class _VendorDetailsScreenState extends ConsumerState<VendorDetailsScreen> {
  ...
  Future<void> _fetchPage(int pageKey) async {
    try {
      ...
      final newItems = await catalogRepository.getVendorById(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          const ProductSearch(),
          if (hasSortOption(_vendor?.catalogProductsModel))
            IconButton(
              icon: const Icon(Icons.sort),
              onPressed: () {
                sortOptionsBuilder(
                  context: context,
                  catalogProducts: _vendor?.catalogProductsModel,
      ...
      body: RefreshIndicator(
        ...
        child: Padding(
          ...
          child: CustomScrollView(
            slivers: [
              if (_vendor?.catalogProductsModel != null)
                SliverToBoxAdapter(
                  child: AppliedFilters(
                    catalogProductsModel: _vendor!.catalogProductsModel!,
                    filterBuilder: _filterBuilder,
                    pagingController: _pagingController,
                  ),
                ),
