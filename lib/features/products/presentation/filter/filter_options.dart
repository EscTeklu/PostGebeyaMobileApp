...
class _FilterOptionsBuilderState extends State<FilterOptionsBuilder> {
  ...
  void initState() {
    ...
    for (var attribute
        in widget.catalog?.specificationFilter?.attributes ??
  ...
  Widget build(BuildContext context) {
    TextStyle headerTextStyle = Theme.of(
      context,
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          context.locale!.catalog_filters,
      ...
      body: Padding(
        ...
        child:
            (widget.catalog?.priceRangeFilter?.enabled ?? false) ||
                ...
                ? SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      if (widget.catalog?.priceRangeFilter?.enabled ??
                          false) ...[
                        ...priceRangeFilter(
                          headerTextStyle,
                          widget
      ...
      attr = Column(
        ...
        children: [
          Padding(
            ...
            child: Text(key, style: headerTextStyle),
          ),
          ...value!.map<Widget>(
            (attribute) =>
                attribute.colorSquaresRgb != null
                    ? CheckboxListTile(
  ...
  List<Widget> manufacturerFilter(
    TextStyle headerTextStyle,
    List<SelectListItemDtoBuilder>? manufactures,
  ) {
    return [
      Padding(
        padding: const EdgeInsets.fromLTRB(10, 15, 10, 5),
  ...
  void _applyFilters() {
    ...
    widget.filterBuilder.specificationOptionIds = ListBuilder(
      specificationOptionIds,
    );

    widget.filterBuilder.manufacturerIds = ListBuilder(
      manufacturers!
          .where((element) => element.selected ?? false)
