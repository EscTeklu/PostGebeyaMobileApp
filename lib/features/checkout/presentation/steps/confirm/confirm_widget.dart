...
class _ConfirmFormContentsState extends ConsumerState<ConfirmFormContents> {
  ...
  Future<void> _submit_BAHU() async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Payment Started')));
    if(paymentMethod == 'ETHIOSWITCH')
      {
        ...
        try {
          String simplified = orderTotal.toString().replaceAll(RegExp(r'[^0-9]'), '');
          int amount = int.parse(simplified); // Now amount = 30900
          //
          final datePart = "${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}";
          final randomPart = random.nextInt(9000) + 1000;
          var id = "ORD-$datePart-$randomPart";
          final res = await paymentService.initiatePayment(amount.toString(), id.toString());
          final orderId = res['orderId'].toString();
          final url = res['redirectUrl'].toString();
          final result = await  showDialog(
            context: context,
            builder:
                (context) => PaymentWebViewScreen(url: url, orderId: orderId, paymentService: paymentService),
          );
          /*final result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => PaymentWebViewScreen(url: url, orderId: orderId, paymentService: paymentService),
            ),
          );*/
        ...
        // Example: ORD-20250722-8392
        //startPayment(orderTotal as double);
        /*final datePart = "${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}";
        final randomPart = random.nextInt(9000) + 1000;
        String simplified = orderTotal.toString().replaceAll(RegExp(r'[^0-9]'), '');
        int amount = int.parse(simplified); // Now amount = 30900
        ...
        var response = await dio.request(
          'https://99de11d6abd9.ngrok-free.app/ethswitch/pay',
          options: Options(
            method: 'POST',
            headers: headers,
          ),
        ...
        if (response.statusCode == 200) {
          ...
          //launchUrlString(response.data.toString());
          showDialog(
            context: context,
            builder:
                (context) => WebInfo(
              urlWeb: response.data.toString(),
            ),
          );
        }
        else {
