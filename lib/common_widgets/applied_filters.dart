...
class AppliedFilters extends StatelessWidget {
  ...
  final FaIcon deleteIcon = const FaIcon(FontAwesomeIcons.xmark, size: 16);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Wrap(spacing: 10, children: items),
    );
  }
  ...
  void deleteManufacturerFilter(String manufacturerId) {
    filterBuilder.manufacturerIds.removeWhere(
      (item) => item.toString() == manufacturerId,
    );
    pagingController.refresh();
  }

  void deleteSpecificationFilter(int specificationOptionId) {
    filterBuilder.specificationOptionIds.removeWhere(
      (item) => item == specificationOptionId,
    );
    pagingController.refresh();
  }
}
